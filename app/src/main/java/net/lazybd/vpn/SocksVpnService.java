...
import static net.lazybd.vpn.BuildConfig.DEBUG;
...
import net.lazybd.vpn.util.Routes;
...
public class SocksVpnService extends VpnService {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (Build.VERSION.SDK_INT >= 26) {
            String NOTIFICATION_CHANNEL_ID = "net.lazybd.vpn";
    ...
    private void stopMe() {
        ...
        try {
            net.lazybd.vpn.util.System.jniclose(mInterface.getFd());
            mInterface.close();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void configure(String name, String route, boolean perApp, boolean bypass, String[] apps, boolean ipv6) {
        ...
        if (!perApp) {
            // Just bypass myself
            try {
                b.addDisallowedApplication("net.lazybd.vpn");
            } catch (Exception e) {
                e.printStackTrace();
            }
        } else {
            if (bypass) {
                ...
                for (String p : apps) {
                    if (TextUtils.isEmpty(p) || p.trim().equals("net.lazybd.vpn")) {
                        continue;
                    }
    ...
    private void start(int fd, String server, int port, String user, String passwd, String dns, int dnsPort, boolean ipv6, String udpgw) {
        ...
        int i = 0;
        while (i < 5) {
            if (net.lazybd.vpn.util.System.sendfd(fd, getApplicationInfo().dataDir + "/sock_path") != -1) {
                mRunning = true;
                return;
            }
